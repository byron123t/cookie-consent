...
def test_split_site_ranks():
    site_ranks = list(range(1003))
    batch_size = 10
    site_ranks_list = split_site_ranks(site_ranks, batch_size)
    print(site_ranks_list)
    assert len(site_ranks_list) == len(site_ranks) // batch_size + 1


if __name__ == '__main__':
    test_split_site_ranks()
    print('Test passed.')
