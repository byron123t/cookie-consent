...
def is_cookie_file_valid(sent_brcookies, page_url: str, consent_cookie: ConsentCookie, prerej_cookies_file, verbose=3):
    if not url_domain_match(page_url, consent_cookie.cookie['domain']):
        if verbose >= 2: print(f"Unmatch url and consent cookie domain:", page_url, consent_cookie.cookie['domain'], f'{prerej_cookies_file.parent.name}/{prerej_cookies_file.name}')
        return False

    if not _contain_cookie(sent_brcookies, consent_cookie):
        if verbose >= 2: print(f"Post rej cookie not contain consent cookie, {prerej_cookies_file.parent.name}/{prerej_cookies_file.name} {consent_cookie.cookie['name']=} {consent_cookie.cookie['domain']=}")
        return False

    return consent_cookie is not None


def read_prerej_sent_cookies_from_file(prerej_cookies_file: Path, verbose=2, debug=False):
    """Map sent cookies to browser cookies and return them."""
    if verbose >= 3: print(f"Process {prerej_cookies_file.name}")
    try:
        prerej_info = json.loads(prerej_cookies_file.read_text())
    except Exception as e:
        print(f"Error loading cookie for site {prerej_cookies_file.parent.name}/{prerej_cookies_file.name}: {e}")
        return []

    br_cookies = pd.DataFrame(prerej_info['browser_cookies'])

    sent_cookies = _parse_cookies(prerej_info['sent_cookies'])
    ...
    site = prerej_cookies_file.parent.name
    for cookie in sent_brcookies:
        cookie['site'] = site
        if 'load_start_time' in prerej_info:
            cookie['load_start_time'] = prerej_info['load_start_time']
            cookie['load_end_time'] = prerej_info['load_end_time']
            cookie['page_url'] = prerej_info['url']

    return sent_brcookies


def read_sent_cookies_by_read_args(cookie_read_args, keep_sent_cookie, verbose=2):
    if verbose >= 2: print("Num prerej cookie files:", len(cookie_read_args))
    ...
    sent_cookies_list = pool.starmap(read_prerej_sent_cookies_from_file, cookie_read_args) #creates a list of the loaded df's
...
def get_read_args_in_site_dir(site_dir: Path, verbose=0):
    ...
    for prerej_cookies_file in site_dir.glob('prerej_cookies.json'):
        yield (prerej_cookies_file, )
...
def read_prerej_sent_cookies_in_scans(scan_dirs: List[Path], keep_sent_cookie=False):
...
def test_get_valid_cookie_files(site_dir: Path, verbose=1):
    ...
    try:
        brcookies_file = (site_dir / 'prerej_browser_cookies.json')
    ...
    for prerej_cookies_file in site_dir.glob('prerej_*_cookies.json'):
        if prerej_cookies_file.name == 'prerej_browser_cookies.json':
            continue

        # TODO: compare with consent cookie's domain.
        subpage_url = json.loads(prerej_cookies_file.read_text())['url']
        if not is_same_suffixed_domain(subpage_url, page_url):
            if verbose >= 1: print(f'{prerej_cookies_file.parent.name}/{prerej_cookies_file.name} not valid')
            return

        # TODO: to check sent cookies. This may be very heavy and maybe just match the consent cookie domain
        # sent_brcookies = read_prerej_sent_cookies_from_file(prerej_cookies_file)
        # if not is_cookie_file_valid(sent_brcookies, page_url, consent_lib_name):
        #     if verbose >= 1: print(f'{prerej_cookies_file} not valid')
        #     return
...
if __name__ == '__main__':
    ...
    read_prerej_sent_cookies_in_scans(scan_dirs)
    ...
    # %%
    # parallel_read_prerej_sent_cookies_in_site_dirs()

# def read_prerej_sent_cookies_for_site(site_dir):
#     # sent_cookies = []
#     # for prerej_cookies_file in site_dir.glob('prerej_*.json'): # / 'prerej_cookies.json'
#     #     sent_cookies.extend(read_prerej_sent_cookies_from_file(prerej_cookies_file))
#     cookies_files = list(site_dir.glob('prerej_*_cookies.json'))
...
#     sent_cookies_list = pool.starmap(read_prerej_sent_cookies_from_file, cookies_files) #creates a list of the loaded df's
#     sent_cookies = [cookie for sent_cookies in sent_cookies_list for cookie in sent_cookies]

#     # for cookie in sent_cookies: # set in read_prerej_file
#     #     cookie['site'] = site_dir.name
#     return sent_cookies

# def read_prerej_sent_cookies(site_dirs):
#     sent_cookies = []
#     for site_dir in tqdm(site_dirs):
#         sent_cookies.extend(read_prerej_sent_cookies_for_site(site_dir))

#     return pd.DataFrame(sent_cookies).drop_duplicates()

# def obsolete_parallel_read_prerej_sent_cookies(data_dir, keep_sent_cookie=False):
...
#     cookies_files = list(data_dir.glob('*/prerej_*.json'))[:]
#     cookies_files = filter_cookies_files(cookies_files)
#     return parallel_read_sent_cookies(cookies_files, keep_sent_cookie)

# def obsolete_parallel_read_prerej_sent_cookies_in_site_dirs(site_dirs: List[Path], keep_sent_cookie=False):
#     """Assume structured data_dir /site / data_files. Could not aggregate sent cookies to reduce computation because we need to map to the same state of browser cookies."""
#     cookies_files = [afile for site_dir in site_dirs for afile in site_dir.glob('*/prerej_*.json')]
#     cookies_files = filter_cookies_files(cookies_files)
#     return parallel_read_sent_cookies(cookies_files, keep_sent_cookie)

# def filter_cookies_files(cookies_files: List[Path]):
#     # Filter out cookie files which do not contain sent cookies.
#     return [f for f in cookies_files if f.name != 'prerej_browser_cookies.json']