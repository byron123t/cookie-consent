...
sites_with_no_inconsistencies_per_region = {region: [] for region in regions}
...
for region in regions:
    for iteration in range(0, 10):
        ...
        try:
            ...
            sites_with_no_inconsistencies = df.groupby('site')['comply'].apply(lambda x: (x == 'comply').all()).sum()
            sites_with_no_inconsistencies_per_region[region].append(sites_with_no_inconsistencies)
        except Exception as e:
            print(f"Error processing {file_path}: {e}")
        
print(sites_with_no_inconsistencies_per_region)
...
plt.tight_layout()
plt.savefig('data/plots/total_cookies_mean.pdf')
plt.show()
...
for condition in conditions:
    ...
    print(condition, 'Normalized Violation Counts (%):')
    print(regions_labels)
    ...
    plt.tight_layout()
    plt.savefig(f'data/plots/mean_normalized_violations_{condition}.pdf')
    plt.show()


with open('data/detector_results.csv', 'w') as f:
...
for region in regions:
    stat, p = stats.shapiro(total_cookies_df[total_cookies_df['Region'] == region]['MeanTotalCookies'])
    print(f'Region: {region}, Shapiro-Wilk p-value: {p}')

# Levene's Test for homogeneity of variances
stat, p = stats.levene(*(total_cookies_df[total_cookies_df['Region'] == region]['MeanTotalCookies'] for region in regions))
print(f'\nLevene’s Test p-value for Mean Total Cookies per Site: {p}')

# Decide which test to use
if p >= 0.05:
    # ANOVA
    f_stat, p_val = stats.f_oneway(*(total_cookies_df[total_cookies_df['Region'] == region]['MeanTotalCookies'] for region in regions))
    print(f'\nANOVA F-statistic: {f_stat}, p-value: {p_val}')
else:
    # Kruskal-Wallis Test
    h_stat, p_val = stats.kruskal(*(total_cookies_df[total_cookies_df['Region'] == region]['MeanTotalCookies'] for region in regions))
...
for condition in conditions:
    ...
    normal = True
    for region in regions:
        stat, p = stats.shapiro(violation_df[violation_df['Region'] == region]['NormalizedViolation'])
        print(f'Region: {region}, Shapiro-Wilk p-value: {p}')
        if p < 0.05:
            normal = False

    # Levene's Test for homogeneity of variances
    stat, p = stats.levene(*(violation_df[violation_df['Region'] == region]['NormalizedViolation'] for region in regions))
    print(f'\nLevene’s Test p-value for Normalized Violation Counts ({condition.capitalize()}): {p}')

    # Decide which test to use
    if normal and p >= 0.05:
        # ANOVA
        f_stat, p_val = stats.f_oneway(*(violation_df[violation_df['Region'] == region]['NormalizedViolation'] for region in regions))
        print(f'\nANOVA F-statistic for {condition.capitalize()}: {f_stat}, p-value: {p_val}')
    else:
        # Kruskal-Wallis Test
        h_stat, p_val = stats.kruskal(*(violation_df[violation_df['Region'] == region]['NormalizedViolation'] for region in regions))
